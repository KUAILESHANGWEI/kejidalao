...
#### 🚀 快捷安装指令模板：
```bash
bash <(curl -sL https://raw.githubusercontent.com/KUAILESHANGWEI/kejidalao/main/kejilion.sh) app [文件名]
```
示例：如果您的配置文件名为 myapp.conf，则调用指令为： bash <(curl -sL https://raw.githubusercontent.com/KUAILESHANGWEI/kejidalao/main/kejilion.sh) app myapp
