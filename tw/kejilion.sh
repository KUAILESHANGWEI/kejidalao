...
dd_xitong() {
		...
		dd_xitong_4() {
		  ...
		  while true; do
			...
			echo "重裝系統"
			echo "--------------------------------"
			echo -e "${gl_hong}注意:${gl_bai}重裝有風險失聯，不放心者慎用。重装预计花费15分钟，请提前备份数据。"
...
docker_ssh_migration() {
	...
	migrate_docker() {
		...
		kj_ssh_read_host_user_port "目標伺服器IP:" "目標伺服器SSH用戶名 [預設root]:" "目标服务器SSH端口 [默认22]: " "root" "22"
...
echo "------------------------"
echo -e "${gl_kjlan}腳本官網:${gl_bai}https://github.com/KUAILESHANGWEI/kejidalao            ${gl_kjlan}GitHub地址:${gl_bai}${gh_https_url}github.com/KUAILESHANGWEI/kejidalao${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "更新日誌"
	echo "------------------------"
	echo "全部日誌:${gh_proxy}raw.githubusercontent.com/KUAILESHANGWEI/kejidalao/main/kejilion_sh_log.txt"
	echo "------------------------"
